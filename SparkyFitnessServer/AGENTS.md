...
- `utils/outboundUrlPolicy.ts` isn't AI-specific despite its naming — `createGuardedFetch(PUBLIC_ONLY_AI_NETWORK_POLICY)` is the general-purpose way to fetch any other user-supplied URL (e.g. `services/calendarService.ts`'s iCal feed fetch) with the same SSRF guard and no admin override, since there's rarely a legitimate reason for a non-AI user-supplied URL to reach internal infrastructure. `createGuardedFetch` forces `redirect: 'manual'` deliberately — a caller that needs to follow redirects must re-validate each hop through the same guarded fetch itself (see calendarService's `fetchWithGuardedRedirects`) rather than let an unguarded auto-follow bypass the check.
