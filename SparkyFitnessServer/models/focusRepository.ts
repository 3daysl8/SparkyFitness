...
const FOCUS_COLS = `id, user_id, domain_id, timeframe, statement, target_type,
  target_value, unit, parent_focus_id, period_date, status, created_at, updated_at`;
  ...
  try {
    const result = await client.query(
      `INSERT INTO focuses (
       ...
       VALUES ($1, $2, $3, $4, COALESCE($5, 'none'), $6, $7, $8, $9, COALESCE($10, 'active'))
...
/**
 * Resolves the active daily focus for `date`, the active weekly focus whose
 * period_date (week-start) covers `date`, and every active long_term focus.
 * `weekStart` is passed in (Monday-based, computed by the service layer) so
 * this stays a pure query function with no date-math of its own.
 */
