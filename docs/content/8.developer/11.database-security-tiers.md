...
| `focus_domains` | User-defined life areas for the Focus hub (health, work, relationships, ...) | Owner-Only | Owner-Only |
| `focuses` | Daily/weekly/long-term intentions, optional numeric/boolean targets, parent-focus chain | Owner-Only | Owner-Only |
| `focus_checkins` | Reflection/progress entries logged against a focus | Owner-Only | Owner-Only |
| `calendar_feeds` | Subscribed iCal (.ics) feed URLs for the Home dashboard agenda | Owner-Only | Owner-Only |
...
| `planned_workouts` | Dated workout intentions (template-generated, manual, or coach-scheduled), distinct from the session that fulfills one | Delegate with `can_manage_diary` | Delegate with `can_manage_diary` or `can_view_reports` |
